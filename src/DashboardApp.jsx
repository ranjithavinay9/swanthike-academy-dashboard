...
export default function DashboardApp() {
  ...
  const selectedAttendanceStudent = useMemo(() => {
    return (
      students.find((student) => student.Student_ID === attendanceStudentId) ||
      null
    );
  ...
  const profileSummary = useMemo(() => {
    ...
    const totalPaid = paymentRows.reduce(
      (sum, row) =>
        sum +
        (Number(String(row.Amount || 0).replace(/[^0-9.-]/g, "")) || 0),
      0
    );
    ...
    const latestPayment = paymentRows.length
      ? paymentRows[paymentRows.length - 1]
      : null;
    const latestAttendance = attendanceRows.length
      ? attendanceRows[attendanceRows.length - 1]
      : null;
  ...
  async function loadData() {
    if (!isConfigured()) {
      setError(
        "Open src/config.js and paste your Apps Script Web App URL into API_BASE."
      );
      return;
    }
  ...
  async function openStudentProfile(student) {
    ...
    try {
      const result = await getJson(
        `${API_BASE}?action=studentProfile&studentId=${encodeURIComponent(
          student.Student_ID
        )}`
      );
  ...
  async function handleSaveAttendance() {
    ...
    try {
      ...
      if (
        selectedStudent &&
        selectedStudent.Student_ID === selectedAttendanceStudent.Student_ID
      ) {
        await openStudentProfile({ Student_ID: selectedStudent.Student_ID });
      }
    } catch (err) {
          ...
          <div className="page-grid">
            <div className="stats-grid">
              <StatCard
                ...
                helper="From Students sheet"
              />
              <StatCard
                label="Payments Recorded"
                value={dashboard.payments_recorded || 0}
                helper="From Payments sheet"
              />
              <StatCard
                label="Attendance Records"
                value={dashboard.attendance_records || 0}
                helper="From Attendance sheet"
              />
            </div>
            ...
            <div className="card">
              <SectionTitle
                ...
                subtitle="Configure the frontend once, then use it daily."
              />
              <ol className="steps-list">
                <li>
                  Open <code>src/config.js</code>.
                </li>
                <li>
                  Paste your Apps Script URL into <code>API_BASE</code>.
                </li>
                <li>
          ...
          <div className={selectedStudent ? "three-column-grid" : "two-column-grid"}>
            <div className="card">
              <SectionTitle
                ...
                subtitle="Live records from the Students sheet"
              />
              <input
                  ...
                  >
                    <div>
                      ...
                      <div className="item-subtitle">
                        {student.Student_ID} • {student.Course_ID} • {student.Batch_ID}
                      </div>
            ...
            <div className="card">
              <SectionTitle
                title="Add Student"
                subtitle="This form writes directly to Google Sheets"
              />
              <form className="form-grid" onSubmit={handleAddStudent}>
                ...
                <label>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setStudentForm({ ...studentForm, Course_ID: e.target.value })
                    }
                  />
                ...
                <label>
                  ...
                  <input
                    type="date"
                    className="text-input"
                    value={studentForm.Join_Date}
                    onChange={(e) =>
                      setStudentForm({ ...studentForm, Join_Date: e.target.value })
                    }
                  />
                </label>
            ...
            {selectedStudent && (
              <div className="card">
                <SectionTitle
                  ...
                  action={
                    <button className="secondary-button" onClick={closeStudentProfile}>
                      Close
                    </button>
                  }
                />
                  ...
                  <div className="profile-summary-grid">
                    ...
                    <div className="mini-stat-card">
                      ...
                      <div className="mini-stat-value">
                        {profileSummary.presentCount}/{profileSummary.attendanceCount}
                      </div>
                    </div>
                  </div>
                )}
                ...
                <div className="profile-section">
                  ...
                  <div className="mini-card">
                    <div>
                      ...
                      {profileSummary?.latestPayment
                        ? `${profileSummary.latestPayment.Amount} on ${String(
                            profileSummary.latestPayment.Payment_Date || ""
                          )}`
                        : "No payment found"}
                    </div>
                    <div>
                      <strong>Latest Attendance:</strong>{" "}
                      {profileSummary?.latestAttendance
                        ? `${profileSummary.latestAttendance.Status} on ${String(
                            profileSummary.latestAttendance.Date || ""
                          )}`
                        : "No attendance found"}
                    </div>
                    <div>
                      <strong>Fee Status:</strong> {profileSummary?.feeStatus || "-"}
                    </div>
                  </div>
                </div>
                ...
                <div className="profile-section">
                  <h3>Fee Details</h3>
                  {feesRows.length ? (
                    feesRows.map((row, idx) => (
                      <div key={idx} className="mini-card">
                        <div>
                          <strong>Net Fee:</strong> {row.Net_Fee}
                        </div>
                        <div>
                          <strong>Paid:</strong> {row.Paid_Amount}
                        </div>
                        <div>
                        ...
                        <div>
                          <strong>Due Date:</strong> {String(row.Due_Date || "")}
                        </div>
                      </div>
                    ))
                  ) : (
                ...
                <div className="profile-section">
                  ...
                  {paymentRows.length ? (
                    [...paymentRows].reverse().map((row, idx) => (
                      <div key={idx} className="mini-card">
                        <div>
                          <strong>Amount:</strong> {row.Amount}
                        </div>
                        <div>
                          <strong>Mode:</strong> {row.Payment_Mode}
                        </div>
                        <div>
                          <strong>Receipt:</strong> {row.Receipt_No}
                        </div>
                        <div>
                          <strong>Date:</strong> {String(row.Payment_Date || "")}
                        </div>
                      </div>
                    ))
                  ) : (
                ...
                <div className="profile-section">
                  ...
                  {attendanceRows.length ? (
                    [...attendanceRows].reverse().map((row, idx) => (
                      <div key={idx} className="mini-card">
                        <div>
                          <strong>Date:</strong> {String(row.Date || "")}
                        </div>
                        <div>
                          <strong>Status:</strong> {row.Status}
                        </div>
                        <div>
                          <strong>Marked By:</strong> {row.Marked_By}
                        </div>
                        <div>
                          <strong>Batch:</strong> {row.Batch_ID}
                        </div>
                      </div>
                    ))
                  ) : (
          ...
          <div className="two-column-grid">
            <div className="card">
              <SectionTitle
                ...
                subtitle="Search, select one student, and save immediately."
              />
              ...
              <div className="single-column-form">
                ...
                <button
                  ...
                  disabled={!attendanceStudentId || savingAttendance}
                >
            ...
            <div className="card">
              ...
              {selectedAttendanceStudent ? (
                <div className="list-stack">
                  <div className="mini-card">
                    <div>
                      <strong>Name:</strong> {selectedAttendanceStudent.Full_Name}
                    </div>
                    <div>
                      <strong>Student ID:</strong>{" "}
                      {selectedAttendanceStudent.Student_ID}
                    </div>
                    <div>
                      <strong>Course:</strong> {selectedAttendanceStudent.Course_ID}
                    </div>
                    <div>
                      <strong>Batch:</strong> {selectedAttendanceStudent.Batch_ID}
                    </div>
                    <div>
                      <strong>Parent:</strong>{" "}
                      {selectedAttendanceStudent.Parent_Name || "-"}
                    </div>
                    <div>
                      <strong>Status to Save:</strong> {attendanceStatus}
                    </div>
                    <div>
                  ...
                  <button
                    className="secondary-button"
                    onClick={() => openStudentProfile(selectedAttendanceStudent)}
                  >
          ...
          <div className="two-column-grid">
            <div className="card">
              <SectionTitle
                ...
                subtitle="Writes to the Payments sheet"
              />
            ...
            <div className="card">
              <SectionTitle
                title="Quick Student List"
                subtitle="Useful while collecting fees"
              />
              <div className="list-stack">
